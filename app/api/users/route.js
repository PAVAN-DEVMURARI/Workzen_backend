...
export async function GET(request) {
  try {
    ...
    // Agar Manager ya Admin hai, toh sabhi users dikhao
...
export async function POST(request) {
  try {
    ...
    // Sirf Admin aur Manager hi user create kar sakte hain
    if (!hasRole(user, ['Admin', 'Manager'])) {
      return NextResponse.json(
        { success: false, error: 'Sirf Admin aur Manager user bana sakte hain' },
        { status: 403 }
      );
    }
    ...
    if (!['Employee', 'Manager', 'Admin'].includes(role)) {
      return NextResponse.json(
        { success: false, error: 'Role Employee, Manager ya Admin hona chahiye' },
        { status: 400 }
      );
    }
